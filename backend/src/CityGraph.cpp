...
void CityGraph::addRoad(int fromNode, int toNode, int weight,
...
std::vector<int> CityGraph::findShortestPath(int startId, int endId,
  ...
  // O(log V) for each extraction, leading to an overall O(E log V) efficiency,
  ...
  if (distances.find(startId) == distances.end() ||
      distances.find(endId) == distances.end()) {
  ...
  while (!pq.empty()) {
    ...
    if (d > distances[u])
      continue;
    if (u == endId)
      break; // Optimization: we found the shortest path to target
...
DistrictSummary CityGraph::getDistrictSummary(int startNodeId) const {
  DistrictSummary summary = {0, 0, {}};
  ...
  // Start recursive DFS
  performDFSSummary(startNodeId, visitedNodes, uniqueStreets, edgeCount);

  summary.totalNodes = static_cast<int>(visitedNodes.size());
  summary.totalEdges =
      edgeCount / 2; // Since it's an undirected graph (bidirectional edges)

  for (const auto &street : uniqueStreets) {
    summary.streets.push_back(street);
  }

  return summary;
}

void CityGraph::performDFSSummary(int u, std::set<int> &visitedNodes,
                                  std::set<std::string> &uniqueStreets,
                                  int &edgeCount) const {
  ...
  if (adjacencyList.count(u)) {
    for (const auto &edge : adjacencyList.at(u)) {
      edgeCount++; // Count every edge connection
      uniqueStreets.insert(edge.streetName);

      if (visitedNodes.find(edge.toNode) == visitedNodes.end()) {
        performDFSSummary(edge.toNode, visitedNodes, uniqueStreets, edgeCount);
      }
    }
  }
}
